...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // Start delivery worker (dequeues and sends messages)
    let worker_delivery_queue = delivery_queue.clone();
    let worker_server_keys = server_keys.clone();
    tokio::spawn(async move {
        ichin_mail::delivery::worker::start_delivery_worker(
