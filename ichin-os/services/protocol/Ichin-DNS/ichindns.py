...
class Registry:
    ...
    def _clean_cname_ptrs(self):
        # Ensure CNAME consistency: if a domain has a CNAME record,
        # remove all other record types for that same domain.
        for domain, records in self._domains.items():
            has_cname = any(r.type == "CNAME" for r in records)
            if has_cname:
                self._domains[domain] = [r for r in records if r.type == "CNAME"]
        # If a domain exists as a CNAME target (value of a CNAME record),
        # remove any conflicting direct records for that target.
        cname_targets = set()
        for records in self._domains.values():
            for r in records:
                if r.type == "CNAME":
                    cname_targets.add(r.value.lower())
        for target in cname_targets:
            if target in self._domains:
                self._domains[target] = [r for r in self._domains[target] if r.type == "CNAME"]
